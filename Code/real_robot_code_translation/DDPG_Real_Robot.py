...
class Noise(object):
    ...
    def get_action_gaussian(self, action):
        ...
        gauss_noise = np.random.normal(0, scale=0.05, size=self.action_dim)
...
class DDPGagent:
    ...
    def __init__(self, env, actor_learning_rate=1e-3, critic_learning_rate=1e-3, gamma=0.99,
                 max_memory_size=50_000, tau=0.001, batch_size=32):

        # -------- Hyper-parameters --------------- #
        self.max_memory_size = max_memory_size
        self.batch_size = batch_size

        self.update_interaction = 3  # number of updates of NN per Episode

        self.gamma = gamma  # discount factor
        self.tau   = tau
        self.hidden_size_critic = [32, 32]
        self.hidden_size_actor  = [64, 64]
